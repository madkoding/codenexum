...
export function jsParse(c: string, f: string): Chunk[] {
  ...
  while ((m = fn.exec(c)) !== null)
    r.push(makeRegexChunk({ id: `${f}:fn:${m[1]}`, file: f, name: m[1], type: "function", line: lineOf(c, m.index), content: `function ${m[1]}(${m[2]})` }, lines, f))
  ...
  while ((m = cl.exec(c)) !== null)
    r.push(makeRegexChunk({ id: `${f}:cls:${m[1]}`, file: f, name: m[1], type: "class", line: lineOf(c, m.index), content: `class ${m[1]}` }, lines, f))
  ...
  while ((m = ex1.exec(c)) !== null)
    r.push(makeChunk({ id: `${f}:exp:${m[1]}`, file: f, name: m[1], type: "export", line: lineOf(c, m.index), content: m[0].trim() }, f))
  ...
  while ((m = ex3.exec(c)) !== null)
    r.push(makeChunk({ id: `${f}:exp:*`, file: f, name: "*", type: "export", line: lineOf(c, m.index), content: m[0].trim() }, f))

  const ex4 = /(?:^|\n)\s*export\s+default\s+(\w+)/g
  while ((m = ex4.exec(c)) !== null) {
    const name = m[1]
    if (!["function", "class", "interface", "type", "enum"].includes(name))
      r.push(makeChunk({ id: `${f}:exp:${name}`, file: f, name, type: "export", line: lineOf(c, m.index), content: m[0].trim() }, f))
  }

  const dec = /(?:^|\n)\s*@(\w+)(?:\s*\(([^)]*)\))?/g
  while ((m = dec.exec(c)) !== null)
    r.push(makeChunk({ id: `${f}:dec:${m[1]}`, file: f, name: m[1], type: "decorator", line: lineOf(c, m.index), content: `@${m[1]}${m[2] ? `(${m[2]})` : ""}` }, f))
...
export function tsParse(c: string, f: string): Chunk[] {
  ...
  while ((m = en.exec(c)) !== null)
    r.push(makeRegexChunk({ id: `${f}:enum:${m[1]}`, file: f, name: m[1], type: "enum", line: lineOf(c, m.index), content: `enum ${m[1]}` }, lines, f))
