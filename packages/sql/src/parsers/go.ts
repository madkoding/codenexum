...
export function goParse(c: string, f: string): Chunk[] {
  ...
  while ((m = fn.exec(c)) !== null)
    r.push(makeRegexChunk({ id: `${f}:fn:${m[1]}`, file: f, name: m[1], type: "function", line: lineOf(m.index), content: `func ${m[1]}(${m[2]})` }, lines, f))

  const st = /(?:^|\n)\s*type\s+(\w+)\s+struct\s*\{/g
  while ((m = st.exec(c)) !== null)
    r.push(makeRegexChunk({ id: `${f}:cls:${m[1]}`, file: f, name: m[1], type: "class", line: lineOf(m.index), content: `struct ${m[1]}` }, lines, f))
  ...
  while ((m = im.exec(c)) !== null)
    r.push(makeChunk({ id: `${f}:imp:${m[1] || m[2]}`, file: f, name: m[1] || m[2], type: "import", line: lineOf(m.index), content: m[0].trim() }, f))
